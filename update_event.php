...
// Check to see if the event_id, title, date, start_time, and finish_time are valid.
if (empty($event_id)) {
    echo json_encode(array(
        "success" => false,
...
$stmt = $mysqli->prepare("UPDATE events SET title=?, date=?, start_time=?, finish_time=?, color=?, calendar_id=? WHERE event_id=? AND created_by=?");
if (!$stmt) {
...
$stmt->bind_param('ssssssss', $title, $date, $start_date_time_str, $finish_date_time_str, $color, $calendar_id, $event_id, $_SESSION['user_id']);
